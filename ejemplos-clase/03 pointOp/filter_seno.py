...
#se aplica el filtro seno a la imagen
q = image_RGB.max()
d = 2 * q
image_RGB_sine = np.array(q * np.sin((np.pi * image_RGB) / d),dtype='uint8')
...
ax2.imshow(image_RGB_sine)
ax2.set_title('Sine Correction')
#se dibujan las graficas de las funciones
x = np.linspace(0, 255, 255)
y1 = x
y2 = np.array(q * np.sin(np.pi * x / d))
